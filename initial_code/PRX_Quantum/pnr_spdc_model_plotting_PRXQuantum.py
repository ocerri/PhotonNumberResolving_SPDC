...
svd = []
with open("JSI_paper_prx_quantum.txt") as csvfile:
...
# fig, axs = plt.subplots(1,1, num = 100)
# width = 0.9
# axs.plot(lambda_index, svd,".k", label = "K = {:.1f}".format(K))
# axs.bar(lambda_index, svd,width,color = "g", alpha = 0.5)#, label = "No Filter (Fit), K = {:2f}".format(K_fil))
...
mu_guess_arr = [1.25803231e-03, 3.22302471e-03, 4.22703968e-03, 5.23692970e-03,6.28560142e-03, 1.42647599e-02, 1.88170101e-02, 1.18375995e-02, 5.27387371e-02, 1.36801453e-01, 3.37982499e-01, 5.13443060e-01,7.55478743e-01, 1.21626065e+00, 1.59783999e+00, 1.82643703e+00, 2.03416430e+00, 2.39732787e+00, 2.93749606e+00, 3.44547132e+00, 3.84130621e+00]
...
popt_idler_singlephoton, pcov_idler_singlephoton= curve_fit(_Ci_pnr_analytic,mu_solve_arr,Ci_singlephoton_data_arr,p0=10)
...
print("eta_s1", eta_s1/2, eta_s1_err/2)
print("eta_s2", eta_s2/2, eta_s2_err/2)
...
title_arr = [[idler_title,twofolds_is1_title], [signals_title,twofolds_is2_title], [twofolds_s1s2_title,threefolds_title]]
...
x = mu_arr
y = Ci_total_data_arr/rate
yerr = Ci_total_err_arr/rate
yhat = Ri_binary_analytic(x,eta_i,lambdas = svd)
...
# lambda_hat = np.sum((Ci_total_data_arr/rate-Ri_binary_analytic(mu_arr,eta_i,lambdas = svd))**2/(Ci_total_err_arr/rate)**2)
# r = chi2.sf(lambda_hat,len(Ci_total_data_arr)-1)

l_hat =  np.sum(np.square((y-yhat)/yerr))
dof = len(x)
pval = chi2.sf(l_hat, dof)
print('Chi2 = {:.1f}/{:.0f} ({:.1f}%)'.format(l_hat, dof, 100*pval))
# print("chi2 for Ri_total: ", r)
print()
...
# lambda_hat = np.sum((Ci_singlephoton_data_arr/rate-Ri_pnr_analytic(mu_arr,eta_i,lambdas = svd, k = k))**2/(Ci_singlephoton_err_arr/rate)**2)
# r = chi2.sf(lambda_hat,len(Ci_singlephoton_data_arr)-1)
# print("chi2 for Ri_single_photon: ", r)
# print()
